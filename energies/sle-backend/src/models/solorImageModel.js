...
const addDailyImage = async (imageUrl, date) => {
  const query = "INSERT INTO solor_image (image_url, date) VALUES (?, ?)";
...
const getAllDailyImages = async () => {
  const query = "SELECT * FROM solor_image";
...
const getDailyImageById = async (id) => {
  const query = "SELECT * FROM solor_image WHERE id = ?";
...
const updateDailyImage = async (id, imageUrl, date) => {
  ...
  if (imageUrl) {
    query = "UPDATE solor_image SET image_url = ?, date = ? WHERE id = ?";
    params = [imageUrl, date, id];
  } else {
    query = "UPDATE solor_image SET date = ? WHERE id = ?";
    params = [date, id];
  }
