...
const addDailyImage = async (imageUrl, date) => {
  const query =
    "INSERT INTO kumbaldaily_images (image_url, date) VALUES (?, ?)";
...
const getAllDailyImages = async () => {
  const query = "SELECT * FROM kumbaldaily_images";
...
const getDailyImageById = async (id) => {
  const query = "SELECT * FROM kumbaldaily_images WHERE id = ?";
...
const updateDailyImage = async (id, imageUrl, date) => {
  ...
  if (imageUrl) {
    query =
      "UPDATE kumbaldaily_images SET image_url = ?, date = ? WHERE id = ?";
    params = [imageUrl, date, id];
  } else {
    query = "UPDATE kumbaldaily_images SET date = ? WHERE id = ?";
    params = [date, id];
  }
