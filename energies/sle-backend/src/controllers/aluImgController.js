import aluminDailyImagesModel from "../models/aluminImageModel.js";
...
const addDailyImage = async (req, res) => {
  ...
  try {
    const imageId = await aluminDailyImagesModel.addDailyImage(imageUrl, date);
    res.status(201).json({
...
const getAllDailyImages = async (req, res) => {
  try {
    const images = await aluminDailyImagesModel.getAllDailyImages();
...
const getDailyImageDetails = async (req, res) => {
  ...
  try {
    const image = await aluminDailyImagesModel.getDailyImageById(id);
    if (!image) {
...
const updateDailyImage = async (req, res) => {
  ...
  try {
    const updatedImage = await aluminDailyImagesModel.updateDailyImage(
      id,
      imageUrl,
      date
    );
