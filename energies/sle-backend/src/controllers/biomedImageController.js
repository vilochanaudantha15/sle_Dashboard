import biomedDailyImagesModel from "../models/biomedImageModel.js";
...
const addDailyImage = async (req, res) => {
  ...
  try {
    const imageId = await biomedDailyImagesModel.addDailyImage(imageUrl, date);
    res.status(201).json({
...
const getAllDailyImages = async (req, res) => {
  try {
    const images = await biomedDailyImagesModel.getAllDailyImages();
...
const getDailyImageDetails = async (req, res) => {
  ...
  try {
    const image = await biomedDailyImagesModel.getDailyImageById(id);
    if (!image) {
...
const updateDailyImage = async (req, res) => {
  ...
  try {
    const updatedImage = await biomedDailyImagesModel.updateDailyImage(
      id,
      imageUrl,
      date
    );
