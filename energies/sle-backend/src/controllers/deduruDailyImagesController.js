import deduruDailyImagesModel from "../models/deduruDailyImagesModel.js";
...
const addDailyImage = async (req, res) => {
  ...
  try {
    const imageId = await deduruDailyImagesModel.addDailyImage(imageUrl, date);
    res.status(201).json({
...
const getAllDailyImages = async (req, res) => {
  try {
    const images = await deduruDailyImagesModel.getAllDailyImages();
...
const getDailyImageDetails = async (req, res) => {
  ...
  try {
    const image = await deduruDailyImagesModel.getDailyImageById(id);
    if (!image) {
...
const updateDailyImage = async (req, res) => {
  ...
  try {
    const updatedImage = await deduruDailyImagesModel.updateDailyImage(
      id,
      imageUrl,
      date
    );
