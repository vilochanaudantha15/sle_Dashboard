import kumbalModelImage from "../models/kumbalModelImage.js";
...
const addDailyImage = async (req, res) => {
  ...
  try {
    const imageId = await kumbalModelImage.addDailyImage(imageUrl, date);
    res.status(201).json({
...
const getAllDailyImages = async (req, res) => {
  try {
    const images = await kumbalModelImage.getAllDailyImages();
...
const getDailyImageDetails = async (req, res) => {
  ...
  try {
    const image = await kumbalModelImage.getDailyImageById(id);
    if (!image) {
...
const updateDailyImage = async (req, res) => {
  ...
  try {
    const updatedImage = await kumbalModelImage.updateDailyImage(
      id,
      imageUrl,
      date
    );
