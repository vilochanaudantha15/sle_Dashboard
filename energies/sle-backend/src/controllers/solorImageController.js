import solorModelImage from "../models/solorImageModel.js";
...
const addDailyImage = async (req, res) => {
  ...
  try {
    const imageId = await solorModelImage.addDailyImage(imageUrl, date);
    res.status(201).json({
...
const getAllDailyImages = async (req, res) => {
  try {
    const images = await solorModelImage.getAllDailyImages();
...
const getDailyImageDetails = async (req, res) => {
  ...
  try {
    const image = await solorModelImage.getDailyImageById(id);
    if (!image) {
...
const updateDailyImage = async (req, res) => {
  ...
  try {
    const updatedImage = await solorModelImage.updateDailyImage(
      id,
      imageUrl,
      date
    );
