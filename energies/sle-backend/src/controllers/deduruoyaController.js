import DeduruOyaModel from "../models/deduruoyaModel.js";
...
export const addDailyData = async (req, res) => {
  try {
    ...
    const insertId = await DeduruOyaModel.addDailyData(date, units);
...
export const getAllDailyData = async (req, res) => {
  try {
    ...
    if (date) {
      ...
      data = await DeduruOyaModel.getDailyDataByDate(date);
    } else {
      // Fetch all daily data
      data = await DeduruOyaModel.getAllDailyData();
    }
...
// Controller to delete daily data by ID
export const deleteDailyData = async (req, res) => {
  try {
    ...
    const affectedRows = await DeduruOyaModel.deleteDailyData(id);
    if (affectedRows === 0) {
